...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <section className="w-full">
            ...
            <div className={`fixed left-0  h-screen transition-all border-r duration-300 ease-in-out ${isOpen ? 'w-[44px] sm:w-32' : 'w-0'}`}>
                <nav className="">

                    <div className={`relative top-1 ${isOpen ? "left-4 sm:left-24 pl-4" : "left-0 pl-0"} transition-all duration-300`}>
                        <button onClick={toggleSidebar}>
                            {
                                isOpen ? <ChevronLeft size={32} className="text-black bg-gray-200 rounded-full p-2 dark:text-black" /> : <ChevronRight size={32} className="text-black bg-gray-200 rounded-full p-2 dark:text-black" />
                            }
                        </button>
                    </div>
                    <div className={`${isOpen ? "flex flex-col gap-2" : "hidden"}`}>
                        {
                            menuItems.map((item, index) => {
                                return (
                                    <Button key={index} variant={`${item.label === "Students" ? "secondary" : "ghost"}`} className="w-full justify-start gap-2">
                                        <div className={`flex flex-col sm:flex-row gap-1 items-center justify-center`}>
                                            {
                                                item.icon
                                            }
                                            <span className="hidden sm:block text-md font-medium text-center">{item.label}</span>
                                        </div>
                                    </Button>
                                )
                            })
...
const menuItems = [
    { id: 1, label: "Dashboard", icon: <Clock className="mr-2 h-4 w-4" />, link: "/dashboard" },
    { id: 2, label: "Students", icon: <Users className="mr-2 h-4 w-4" />, link: "/projects" },
    { id: 3, label: "Chapters", icon: <BookOpen className="mr-2 h-4 w-4" />, link: "/pathways" },
    { id: 4, label: "Help", icon: <QuestionMark className="mr-2 h-4 w-4" />, link: "/opensource" },
    { id: 4, label: "Reports", icon: <BarChart className="mr-2 h-4 w-4" />, link: "/opensource" },
    { id: 4, label: "Settings", icon: <Settings className="mr-2 h-4 w-4" />, link: "/opensource" },
]
